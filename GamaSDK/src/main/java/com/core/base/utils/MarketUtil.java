...
public class MarketUtil {
    ...
    */
    ...
    public static void toGooglePaly(Activity activity, String googlePayUrl) {
        ...
        String marketUrl = googlePayUrl;
        if (SStringUtil.isNotEmpty(googlePayUrl) && googlePayUrl.startsWith("https://play.google.com/store/apps/details?id=")){
             marketUrl = googlePayUrl.replace("https://play.google.com/store/apps/details?id=","market://details?id=");
        }

        try {

            Intent intent = new Intent(Intent.ACTION_VIEW);
            intent.setPackage("com.android.vending");   //设置市场商店为Google商店，优先使用Google商店打开
            ...
            if (intent.resolveActivity(activity.getPackageManager()) != null){
                ...
                Intent intent2 = new Intent(Intent.ACTION_VIEW);
                intent2.setData(Uri.parse(marketUrl)); //跳转到应用市场，非Google Play市场一般情况也实现了这个接口
                intent2.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
