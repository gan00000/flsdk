...
import com.ldy.sdk.pay.gp.res.GPCreateOrderIdRes;
import com.ldy.sdk.pay.gp.res.GPExchangeRes;
import com.ldy.sdk.pay.gp.task.LoadingDialog;
import com.ldy.sdk.pay.gp.util.GBillingHelper;
import com.ldy.sdk.pay.gp.util.PayHelper;
                ...
                for (com.android.billingclient.api.Purchase purchase : list) {//查询是否为PURCHASED未消费商品
                    if(purchase.getPurchaseState() == com.android.billingclient.api.Purchase.PurchaseState.PURCHASED){
                        ...
                        mHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                PayApi.requestSendStone(mContext, purchase, "yes", new SFCallBack<GPExchangeRes>() {
                                    @Override
...
//            只有在状态为 PURCHASED 时，您才能授予权利。请使用 getPurchaseState()（而非 getOriginaljson()），并确保正确处理 PENDING 交易。
            if(purchase.getPurchaseState() == Purchase.PurchaseState.PURCHASED){
                ...
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        PayApi.requestSendStone(activity, purchase,"yes", new SFCallBack<GPExchangeRes>() {
                            @Override
                            public void success(GPExchangeRes result, String msg) {
