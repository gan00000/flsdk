...
import com.mybase.cipher.DESCipher;
...
public class SdkUtil {
    ...
    public static ConfigModel getSdkCfg(Context context){
        String cfg = getSdkCfgString(context);
        if (JsonUtil.isJson(cfg)){
            try {
                Gson gson = new Gson();
                configModel = gson.fromJson(cfg, ConfigModel.class);
                return configModel;
...
//    private final static String cipherPasswordFlag = "888*****888";
    ...
    public static void updateLoginData(Context context, SLoginResult sLoginResponse){
    ...
    public static SLoginResult getCurrentUserLoginResponse(Context context){
        ...
        try {
            Gson gson = new Gson();
            return gson.fromJson(loginResult, SLoginResult.class);
        } catch (JsonSyntaxException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static boolean isLogin(Context context){
        SLoginResult sLoginResponse = getCurrentUserLoginResponse(context);
...
//        为防止上次獲取谷歌ID下次獲取安卓ID的情况，獲取的ID都保存到本地，後面從本地去讀取
    ...
    public static void saveSUserInfo(Context context, SLoginResult loginResponse) {
        if (loginResponse != null && loginResponse.getData() != null){
            ...
            newUserInfo.setFirstLoginTime(loginResponse.getData().getTimestamp());
