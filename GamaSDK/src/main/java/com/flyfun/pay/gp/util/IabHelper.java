...
package com.flyfun.pay.gp.util;
...
public class IabHelper {
	...
	public static final String mDebugTag = "iabHelper";

	// Is setup done?
	/**
	 * mSetupDone 是否已经启动远程服务
	 */
...
//		void onError(String message);
	...
	public void startSetup(final OnIabSetupFinishedListener listener) {
		...
		// If already set up, can't do it again.
		// if (mSetupDone) throw new IllegalStateException("IAB helper is
		// already set up.");
		if (mSetupDone) {
			return;
		}
...
//							listener.onError("Google Play not support billing v3,update Google Play please");
				...
				/*	response = mService.isBillingSupported(3, packageName, ITEM_TYPE_SUBS);//检查订阅是否支持购买
					if (response == BILLING_RESPONSE_RESULT_OK) {
...
//				listener.onError("Google Play unavailable on device");
	...
	/*
	 * private void checkNotDisposed() { if (mDisposed) throw new
	 * IllegalStateException("IabHelper was disposed of, so it cannot be used."
	 * ); }
	 */
	...
	public synchronized void launchPurchaseFlow(Activity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener, String extraData) {
	...
	public void launchPurchaseFlow(Activity act, String sku, String itemType, int requestCode, OnIabPurchaseFinishedListener listener,
		...
		try {
			...
			Bundle buyIntentBundle = mService.getBuyIntent(3, context.getPackageName(), sku, itemType, extraData);
...
//				}
//			}
//			PayHelper.saveCurrentOrderId(act, context.getGoogleOrderBean().getOrderId());
...
//			verifyTask.verifyOrder(context, mPurchaseListener, purchaseData, dataSignature, mPurchasingItemType);
	...
	/**
	 * <p>
	 * Description: 查询未消费的商品和商品详情
	 * </p>
	 * 
	 * @param querySkuDetails
	 *            是否需要查询详情
	 * @param moreSkus
	 *            出了未消费的商品，额外需要查询情况的商品id
	 * @return
	 * @throws IabException
	 * @date 2015年1月21日
	 */
...
//			context.getPayDialog().dismissProgressDialog();
	...
	void checkSetupDone(String operation) {
		if (!mSetupDone) {
			// context.getPayDialog().complainCloseAct("Google儲值未初始化完成，請重新進入");
//			context.getPayDialog().complainCloseAct("Google IAB set up error，please try again");
	...
	void flagStartAsync(String operation) {
		if (mAsyncInProgress) {
//			context.getPayDialog().dismissProgressDialog();
			SLog.logD("Can't start async operation (" + operation + ") because another async operation(" + mAsyncOperation + ") is in progress.");
			return;
	...
	int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
		...
		do {
			...
			for (int i = 0; i < purchaseDataList.size(); ++i) {
				...
				SLog.logI("验证查询未消费的订单，sku: " + sku);
				boolean s = true;
//				if (verifyTask.verifyQueryInventory(context, purchaseData, signature, null)) {
...
//			context.getPayDialog().dismissProgressDialog();
		...
		(new Thread(new Runnable() {
			public void run() {
				...
				flagEndAsync();
				/*
				 * if (singleListener != null) { handler.post(new Runnable() {
				 * public void run() {
				 * singleListener.onConsumeFinished(purchases.get(0),
				 * results.get(0)); } }); } if (multiListener != null) {
				 * handler.post(new Runnable() { public void run() {
